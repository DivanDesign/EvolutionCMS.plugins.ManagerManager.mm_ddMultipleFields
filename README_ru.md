...
* Генерация уникального идентификатора каждой строки (`$params->columns[i]['type']` == `'id'`).
...
* `$params->columns[i]['type']`
	...
	* Допустимые значения:
		...
		* `'file'` — колонка-файл
		* `'date'` — колонка-дата
		* `'id'` — скрытая колонка, содержащая уникальный ID строки (генерируется автоматически)
